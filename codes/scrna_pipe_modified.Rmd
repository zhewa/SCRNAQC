...
plot.stats.sam <- function(res.table, umi.length) {
  # Calculate relative abundance of UMIs
  umi.table <- table(res.table[,umi])
  umi.inferred.table <- table(res.table[,inferred_umi])
  
  # Base percentage of UMIs for every read
  base.matrix <- c()
  for(i in 1:umi.length) {
    # consider all reads
    #rbindlist(list(num.pcr.products, get.pcr.duplicates(rdt)),
  ...
  ggplot(num.pcr.products, aes(num)) + geom_histogram(bins=30) + theme_Publication() +
    ggtitle(paste0("Number of PCR products per IVT fragment for ", fname,".sam")) +
    xlab("Number of PCR products") + ylab("Frequency")
...
stats <- c(nrow(res.table), 
          ...
          nrow(num.pcr.products)/nrow(res.table),
